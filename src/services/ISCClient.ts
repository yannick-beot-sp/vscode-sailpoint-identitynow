...
export class ISCClient {
	...
	*/
	...
	public async updateRoleMetadata(id: string, attributes: Array<AttributeDTO>) {
		...
		public async getPaginatedDimensions(roleId:string, filters?: string, limit?: number, offset?: number, count = false): Promise<AxiosResponse<DimensionV2025[]>> {

		const apiConfig = await this.getApiConfiguration();
		const api = new DimensionsV2025Api(apiConfig, undefined, this.getAxiosWithInterceptors());
		const response = await api.listDimensions({
			roleId,
			filters,
			limit,
			offset,
