...
export class ISCClient {
	...
	public async getTransformById(id: string): Promise<TransformRead> {
		...
		const apiConfig = await this.getApiConfiguration();
		const api = new TransformsApi(apiConfig, undefined, this.getAxiosWithInterceptors());
		const response = await api.getTransform({ id });
		return response.data;
	}

	public async createTransform(body: TransformV2025): Promise<TransformRead> {
		console.log("> createTransform", body.name);
		const apiConfig = await this.getApiConfiguration();
		const api = new TransformsApi(apiConfig, undefined, this.getAxiosWithInterceptors());
		const response = await api.createTransform({ transform: body });
		return response.data;
	}

	public async updateTransform(id: string, body: TransformV2025): Promise<TransformRead> {
		console.log("> updateTransform", id);
		const apiConfig = await this.getApiConfiguration();
		const api = new TransformsApi(apiConfig, undefined, this.getAxiosWithInterceptors());
		const response = await api.updateTransform({ id, transform: body });
		return response.data;
	}
	...
	public async updateAccessProfile(id: string, operations: Array<JsonPatchOperationV2025>) {
		const apiConfig = await this.getApiConfiguration();
		const api = new AccessProfilesV2025Api(apiConfig, undefined, this.getAxiosWithInterceptors());
